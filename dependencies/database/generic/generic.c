...
int screateFile(char name[MAX_FILE_NAME_LEN], char parentPath[MAX_PATH_LEN])
{
    ...
    if ( strcpy(newFile.name, name) == NULL ) return 0;
    if ( strcpy(newFile.path, parentPath) == NULL ) return 0;
    if ( newFile.path[strlen(newFile.path) - 1] != PATH_SEPARATOR)
        if ( strcat(newFile.path, PATH_SEPARATOR_S) == NULL ) return 0;
    if ( strcat(newFile.path, name) == NULL ) return 0;

    result = createFile(&newFile);
    return result;
}
...
int screateFolder(char name[MAX_FILE_NAME_LEN], char parentPath[MAX_PATH_LEN])
{
    ...
    initFolder(&newFolder);

    if ( name[0] == PATH_SEPARATOR ) return 0;
    if ( strcpy(newFolder.name, name) == NULL ) return 0;
    if ( strcpy(newFolder.path, parentPath) == NULL ) return 0;
    if ( newFolder.path[strlen(newFolder.path) - 1] != PATH_SEPARATOR)
        if ( strcat(newFolder.path, PATH_SEPARATOR_S) == NULL ) return 0;
    if ( strcat(newFolder.path, name) == NULL ) return 0;

    newFolder.numFiles = newFolder.numFolders = 0;

    result = createFolder(&newFolder); 
}
